/*
 * File: _coder_auto_corr_analysis_api.c
 *
  ...
  mxArray *plhs[2])
{
  ...
  emxInit_real_T(&st, &lags2, 2, true);
  ...
  emxFree_real_T(&x);
  if (nlhs > 1) {
    lags2->canFreeData = false;
    plhs[1] = emlrt_marshallOut(lags2);
  }

  emxFree_real_T(&lags2);
...
/*
 * File trailer for _coder_auto_corr_analysis_api.c
 *
 * [EOF]
 */
